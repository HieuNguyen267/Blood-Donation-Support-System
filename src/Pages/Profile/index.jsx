...
import { Card, Descriptions, Typography, Button, Row, Col, Form, Input, DatePicker, Select, Radio, Space } from 'antd';
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    if (storedInfo) {
      ...
      form.setFieldsValue(parsedInfo); // Khởi tạo giá trị cho form
    }
  }, [form]);

  const handleEdit = () => {
    // Đảm bảo form được điền đúng dữ liệu từ state trước khi hiển thị
  ...
  const handleSave = (values) => {
    ...
    if (updatedInfo.dob) {
      updatedInfo.dob = moment(updatedInfo.dob);
    }
  ...
  const handleCancel = () => {
    form.setFieldsValue(userInfo); // Reset form về giá trị ban đầu
    setIsEditMode(false);
  };
