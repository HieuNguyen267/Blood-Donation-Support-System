...
export default function Statistics() {
    ...
    const fetchStats = async () => {
      try {
        ...
        setDashboardStats({
          ...
          donorsThisMonth: countDonorsThisMonth(donors),
          requestsThisMonth: countRequestsThisMonth(requests),
  ...
  const donorsByDay = days.map(day =>
    dashboardStats.donationStats.filter(d => {
      if (!d.last) return false;
      const date = new Date(d.last);
  ...
  const requestsByDay = days.map(day =>
    dashboardStats.requestStats.filter(r => {
      if (!r.required_by) return false;
      const date = new Date(r.required_by);
      return date.toDateString() === day.toDateString();
    }).length
          ...
          <div className="row" style={{marginBottom:32}}>
            <div className="col-md-6">
              <div className="stat-card" style={{background:'#e0f7fa'}}>
                ...
                <div style={{fontSize:36, fontWeight:700, color:'#1976d2'}}>{dashboardStats.donorsThisMonth}</div>
              </div>
            </div>
            <div className="col-md-6">
              <div className="stat-card" style={{background:'#fff7e0'}}>
                <div style={{fontSize:18, color:'#fb923c'}}>Đơn yêu cầu máu trong tháng</div>
                <div style={{fontSize:36, fontWeight:700, color:'#fb923c'}}>{dashboardStats.requestsThisMonth}</div>
              </div>
            </div>
          </div>
